...
class GameState(object):
    ...
    def __init__(self, size=19):
        ...
        self.consecutive_passes = 0  # Game ends on 2 consecutive pass moves
    ...
    def do_move(self, action):
        ...
        if self.is_legal(action):
            ...
            if action is not PASS_MOVE:
                ...
                for (nx, ny) in self._neighbors(action):
                    if self.board[nx, ny] == -self.current_player and len(
                        self.liberty_sets[nx][ny]) == 0:
                        # capture occurred!
                ...
                # Penalty for passing
                if self.current_player == BLACK:
                    self.num_black_prisoners += 1
                else:
                    self.num_white_prisoners += 1
                # Keep track of number of consecutive passes
        ...
        end_of_game = self.consecutive_passes >= 2 and self.current_player == WHITE
        return end_of_game
